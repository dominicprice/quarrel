...
class Cells {
	...
	static fromTemplate = (template: string[]) => {
		...
		for (let i = 0; i < template.length; ++i) {
			for (let j = 0; j < template[i].length; ++j) {
				if (template[i][j] !== ".") cells.setValue([i, j], template[i][j]);
			}
		}
		return cells;
	};
	...
	setValue = (pos: Position, value: string) => {
		...
		if (value === "") {
			...
			if (this.cells[sRow][sCol].isEmpty()) this.cells[sRow][sCol].value = "?";
		}
	...
	toggleSplit = (pos: Position, dir: Dir, split: Split) => {
		...
		if (dir === Dir.Across) {
			...
			if (pos[0] > 0)
				cell.splitAbove = cell.splitAbove === split ? Split.None : split;
		}
